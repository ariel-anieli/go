...
func cgoFormatErr(error cgoErrorMsg) errorString {
	pcs := make([]uintptr, 10)
	var msg string
	...
	if error == cgoResultFail {
		...
		msg += "is unpinned Go pointer or points to unpinned Go pointer"
	} else if error == cgoCheckPointerFail {
		msg = "cgo argument of function" + callerName
		msg += "has Go pointer to unpinned Go pointer"
	}
